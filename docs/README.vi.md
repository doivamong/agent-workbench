...
| **Công bố một lát memory an toàn công khai** | Một sync được leak scan canh, **fail-closed** — chỉ chép fact đánh dấu `visibility: public` (hoặc đã publish) mà qua được `leak_scan`, vào `memory/` của repo công khai; gỡ frontmatter per-session, để index do người tự sửa, chỉ chạy thủ công | [`tools/memory_sync.py`](../tools/memory_sync.py) |
