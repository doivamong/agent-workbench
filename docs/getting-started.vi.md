...
<!-- en-sha256: ba665f2a4e1eab469bbcac0fefef738fb9100edb286049d471f57103a7300e77 leak-scan: ignore[high_entropy_hex] -->
...
- **Python ≥ 3.10.** Kiểm tra bằng `python --version` (trên Windows thử thêm `py --version`); nếu
  thiếu hoặc cũ hơn, cài từ [python.org](https://www.python.org/downloads/). Lõi tái dùng chỉ dùng
  stdlib; `pytest` chỉ dành cho bộ test (CI chạy 3.10 / 3.11 / 3.12).
- **Git** — kiểm tra bằng `git --version`; nếu thiếu, tải từ
  [git-scm.com](https://git-scm.com/downloads). **Tuỳ chọn:** [Claude Code](https://claude.com/claude-code)
  (hoặc agent khác) để dùng hooks và skills — đây là cách điều khiển kit nếu bạn không muốn tự gõ lệnh.
...
> cho người không lập trình.** Không sửa settings, không đọc traceback:
> - Tải kit về máy: trên GitHub bấm **Code ▾ → Download ZIP** rồi giải nén (hoặc `git clone` nếu
>   bạn biết cách).
> - Mở thư mục `agent-workbench` vừa giải nén đó trong Claude Code.
> - Nói: *"cài agent-workbench vào `<đường-dẫn-dự-án-của-tôi>` và xác nhận các guard đã bật."*
>   Chưa rõ đường dẫn? Nói trước: *"giúp tôi tìm thư mục dự án."*
> - Agent (skill `awb-install-and-verify`) chạy lệnh cài bên dưới, rồi `install.py … --doctor` để
>   chứng minh guard chạy, và nói thật cái gì được bảo vệ, cái gì không. Nó chạy **từ thư mục kit**,
>   không phải từ trong dự án trống (ở đó chưa có tool).
...
và — với `--merge-settings` — tự wire hooks cho bạn, nên guard bật ngay trong một lệnh (không phải
sửa file). Bỏ cờ đó thì nó in một đoạn `settings.json` để bạn tự dán vào
`your-project/.claude/settings.json`; `--merge-settings` là đường đơn giản hơn. Thêm `--with-git-hook`
để cài thêm một git pre-commit gate chống rò rỉ.
