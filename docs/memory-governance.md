...
| **Fresh-clone memory seed tool** (install-time *or* opt-in CLI that pre-fills a new user's live dir) | A fresh clone's empty derived recall is the **honest day-1 state** (§1), not waste — the committed `memory/` template + install wiring + a loud `memory_recall_doctor` serve read-once onboarding, and a user's *own* facts accumulate via the existing plain-language `awb-lessons-capture` skill. Pre-seeding the author's craft adds **overload** (§8: *redundancy* — Tier-1 reasoning already lives in `CLAUDE.md`/`.claude/rules`; *relevance* — the rest is dev scar-tissue irrelevant to the user's project). Seed-at-install also re-derives the version-fragile cwd-mangle at package time → silent no-op into a dead/foreign `~/.claude` dir (§6 verbatim) — **rejected**. And a seed is a new live-dir **mutator** (trips the snapshot-precondition row), a partial **index generator** (deferred write-tool), and a 7th memory tool (deepens the already-reached `tools/memory/` package trigger). | Build only after **both**: (a) ≥2–3 recorded recall-miss / abandonment incidents from real fresh-clone **non-author** users who, despite the loud docs + `awb-lessons-capture` + `recall_doctor`, demonstrably got zero day-1 value; **and** (b) `resolve_live_dir` stat-verifies a writable live dir on a second, non-author machine **and** the snapshot-before-mutation code-half (the row above) has shipped. If ever built: reuse `resolve_live_dir`/`parse_frontmatter`/`memory_budget` (never re-derive), resolve at runtime + refuse-loud if unresolved, Tier-1 allowlist only, hard-exclude the `*_example_*` files, no-clobber, self-prove via `recall_doctor`; prefer extending `memory_snapshot.py` over a 7th tool. | this register (incident) + the new `recall_doctor` day-1-empty WARN (a measurable companion, so the defer is not a reject-in-disguise) + the `defer-discipline` rule (fires when a memory tool is added) | 2026-06-07 |
