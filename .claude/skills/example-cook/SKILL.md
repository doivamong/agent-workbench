...
description: >
  ...
  DO NOT TRIGGER: a routine single-file change (use example-plan-then-code); a one-line fix; a pure
...
> **Announce on activation:** "Using example-cook — pick an oversight mode, plan from several angles,
> then build through the guard skills."

For an ordinary change, [`example-plan-then-code`](../example-plan-then-code/SKILL.md) is the right,
...
4. **Implement** in small steps; hand non-trivial logic to [`example-tdd`](../example-tdd/SKILL.md).
5. **Run the guards at their step** (don't reinvent them): long generation →
   [`example-output-guard`](../example-output-guard/SKILL.md); config access →
   [`example-config-guard`](../example-config-guard/SKILL.md); before commit →
   [`example-review`](../example-review/SKILL.md).
