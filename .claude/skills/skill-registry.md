...
| `awb-handover` | workflow | ending a session, "hand this off / package for next session / write a handover", research-done-move-to-execution | work not finished enough to hand off; a quick mid-task note; starting fresh implementation now; you'll resume THIS same conversation later (history comes back — handover is for a COLD reader: new/cleared session, teammate, other machine) |
| `awb-stress-test` | workflow | "what could go wrong / is this design sound / edge cases" before building or testing something non-trivial ("stress test this", "poke holes in this plan") | a one-line / low-risk change; comparing approaches (research); reviewing code already written (review); a known bug (debug) |
...
| `awb-cook` | workflow | a larger / multi-module / hard-to-reverse change where one plan would hide blind spots, or you want to dial checkpoint frequency or plan from several independent angles ("cook this", "run the full workflow") | a routine single-file change (plan-then-code); a one-line fix; a question; no sub-agent fan-out wanted |
...
| `awb-verify-docs` | feature | writing/changing code against a specific library/framework/API (a hook signature, a config key, a version-sensitive pattern), "does this library do X" | choosing which library/approach (awb-research); a reuse/license decision (awb-external-ref); original logic with no framework/version surface |
