...
def test_discover_skill_names_needs_skill_md(tmp_path):
    d = _skills(tmp_path, "awb-debug")
    (d / "no-skill-md").mkdir()  # a directory without SKILL.md is not a skill
    assert logger.discover_skill_names(d) == ["awb-debug"]


def test_discover_skill_names_longest_first(tmp_path):
    d = _skills(tmp_path, "review", "awb-review")
    assert logger.discover_skill_names(d) == ["awb-review", "review"]
...
def test_find_signals_invoke_vs_mention():
    names = ["awb-review", "awb-debug"]
    assert logger.find_signals("please /awb-review this", names) == [("awb-review", "invoke")]
    assert logger.find_signals("use awb-debug now", names) == [("awb-debug", "mention")]


def test_find_signals_word_boundary_no_false_match():
    # the common word "review" must NOT match the skill "awb-review"
    assert logger.find_signals("review my code", ["awb-review"]) == []


def test_find_signals_each_skill_once_at_strongest():
    out = logger.find_signals("/awb-debug and awb-debug again", ["awb-debug"])
    assert out == [("awb-debug", "invoke")]
...
def test_logger_writes_one_line_per_signal_and_no_prompt_text(tmp_path):
    _skills(tmp_path, "awb-debug")
    log = tmp_path / "u.jsonl"
    proc = _run_logger(json.dumps({"prompt": "run /awb-debug now", "session_id": "abc"}),
    ...
    assert len(lines) == 1
    assert lines[0]["skill"] == "awb-debug"
    assert lines[0]["signal"] == "invoke"
    assert len(lines[0]["prompt"]) == 8          # an 8-char digest, not the text
    assert "run /awb-debug now" not in raw   # the prompt text is never stored


def test_logger_kill_switch(tmp_path):
    _skills(tmp_path, "awb-debug")
    log = tmp_path / "u.jsonl"
    proc = _run_logger(json.dumps({"prompt": "/awb-debug"}),
                       {"CLAUDE_PROJECT_DIR": str(tmp_path), "SKILL_USAGE_LOG_PATH": str(log),
                        "SKILL_USAGE_LOG": "0"})
...
def test_discover_known_skills(tmp_path):
    d = _skills(tmp_path, "awb-debug", "deep-research")
    assert report.discover_known_skills(d) == {"awb-debug", "deep-research"}
