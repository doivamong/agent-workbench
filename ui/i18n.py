...
_UI = {
    "vi": {
        ...
        "mem_day1_empty": "Recall sống đang TRỐNG (bản mới clone) — đang hiển thị template của repo. Agent chưa nhớ gì về dự án; gõ “capture the lessons” để bắt đầu.",
        ...
        "admin_reach_localhost": "chỉ máy này",
        "admin_reach_lan": "truy cập được trong LAN (chỉ đọc)",
        "admin_reach_other": "bind tùy chỉnh",
        "admin_lan_enable_label": "Bật mặc định LAN",
        "admin_lan_enable_confirm": "Việc này làm dashboard chỉ-đọc truy cập được trong mạng LAN. Khi đã đặt mật khẩu admin thì /admin cũng truy cập được và mật khẩu đi qua HTTP dạng CLEARTEXT — chỉ làm trong mạng tin cậy, và dùng reverse proxy / Cloudflare Tunnel nếu muốn truy cập từ domain. Đặt AWB_DASHBOARD_HOST=0.0.0.0 (setx) và áp dụng; khởi động lại để có hiệu lực?",
        "admin_lan_disable_label": "Tắt mặc định LAN",
        "admin_lan_disable_confirm": "Quay về localhost-only (chỉ máy này truy cập được dashboard)?",
        "admin_fw_open_label": "Mở firewall LAN",
        "admin_fw_open_confirm": "Mở firewall Windows cho cổng dashboard, chỉ phạm vi LOCAL SUBNET (không bao giờ ra Internet công cộng). Cần quyền admin. Tiếp tục?",
    ...
    "en": {
        ...
        "mem_day1_empty": "Live recall is EMPTY (fresh clone) — showing the repo template. The agent recalls nothing about your project yet; say “capture the lessons” to start.",
        ...
        "admin_reach_localhost": "this machine only",
        "admin_reach_lan": "reachable on LAN (read-only)",
        "admin_reach_other": "custom bind",
        "admin_lan_enable_label": "Enable LAN default",
        "admin_lan_enable_confirm": "This makes the read-only dashboard reachable on your LAN. Once an admin password is set, /admin is reachable too and its password travels in CLEARTEXT over HTTP — only do this on a trusted network, and use a reverse proxy / Cloudflare Tunnel for access from a domain. Set AWB_DASHBOARD_HOST=0.0.0.0 (setx) and apply; restart to take effect?",
        "admin_lan_disable_label": "Disable LAN default",
        "admin_lan_disable_confirm": "Back to localhost-only (only this machine can reach the dashboard)?",
        "admin_fw_open_label": "Open LAN firewall",
        "admin_fw_open_confirm": "Open the Windows firewall for the dashboard port, scoped to your LOCAL SUBNET only (never the public Internet). Needs admin. Continue?",
