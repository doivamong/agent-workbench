...
**guarded tree-restore** — reusing those callable APIs (no second implementation). Without the
flag the blueprint is never registered, so every `/admin*` path is a plain **404**.
