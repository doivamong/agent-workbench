...
# The doctor (verify the wired guards run here) lives in tools/doctor.py so it can
# travel INTO an adopter project (COPY_MAP group `tools`) and run standalone there.
# install.py imports it back, so `install.py --doctor` and the tests keep working off
# one source of truth. tools/doctor.py is stdlib-only and self-contained by design.
sys.path.insert(0, str(KIT / "tools"))
from doctor import (  # noqa: E402
    run_doctor, _probe_interpreter, _parse_hook_command, _interp_argv,
    DOCTOR_BLOCK_LIMIT, DOCTOR_RESTART_NOTE, DOCTOR_LEGEND,
)
...
COPY_MAP = [
    ...
    ("tools/doctor.py", "tools/doctor.py", "tools"),
