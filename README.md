...
| Tests | **757**, green in CI (incl. adversarial evasion cases for the command guard) |
| Runnable demos | **27** (`examples/`) |
| Skills | **17** (10 workflow + 4 guards + 1 meta + 1 feature + 1 audit) |
| Standalone tools | **19** (`invariants`, `affected_tests`, `leak_scan`, `license_scan`, `secrets_guard`, `memory_audit`, `memory_snapshot`, `memory_recall_doctor`, `memory_budget`, `memory_sync`, `memory_eval`, `skill_lint`, `check_context_budget`, `check_requirements_diff`, `sync_manifest`, `skill_usage_report`, `readme_metrics`) |
...
**Agent Workbench** · stdlib-only core · 757 tests · MIT
